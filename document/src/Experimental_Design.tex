...
By default, Docker currently  uses the AUFS file system for its back-end storage driver.
...
Both zfs and btrfs have stability issues and are not yet standard file system drivers so they were also disqualified.
...
Standard kernels usually boot with only a few parameters passed to the kernel such as \lstinline{ro quiet} which will make the kernel read-only and suppress most kernel boot messages.  
...
These resource conflicts can be all but eliminated by ``unplugging'' pCPUs assigned to a guest from the host scheduler with the \lstinline{isolcpus} kernel boot parameters.
...
The Docker environment is logically separated into building with \lstinline{docker build}, back-end management performed by the \texttt{docker daemon}, and the \lstinline{docker run} command that actually instantiates containers from images and assigns the appropriate cgroups and namespacing to the containers.
...
A Dockerfile usually specifies the base image that should be used for the build along with setting environment variables and installing necessary packages and binaries to the container's file system. 
...
The daemon has a number of parameters that may be modified to tune parameters such as available cgroup CPU sets, bridged networking, and the storage driver used for container file systems.
In this study, the \texttt{docker daemon} was launched with mostly standard parameters but for the following exceptions.
As discussed in Section~\ref{sub:software_environment}, the overlayfs storage driver was chosen, requiring the \texttt{docker daemon} to be launched with \lstinline{--storage-driver=overlay}.  
Additionally, Docker's default bridge, \texttt{docker0}, was configured to draw on a pool of IP addresses from the subnet 192.168.42.240/28 for its bridged virtual Ethernet interfaces.
The additional arguments used with \lstinline{docker run} were only used to assign the cgroup cpusets and corresponding memory node for the realtime kernel.  
These arguments comprised \lstinline{--cpuset-cpus=1-4} which tells Docker to create a cgroup containing those CPUs and assign the container to that cgroup and \lstinline{--cpuset-mems=0} which tells Docker to assign socket 0 (the local socket) as the only available memory node.

All containers were run with non-persistent file systems by using the \lstinline{--rm} option in the \texttt{docker run} line.
...
The experimental setup described in Section~\ref{sec:experimental_setup} was designed to enable the performance comparison of virtual machines and containers for network workloads.  
The intent of this study was to compare the performance of the systems with tuning to those without and compare the performance of two different network interfaces.
...
For each of the netperf tests, 20 replicate samples were collected in series.  For the ping test, 100 samples were collected with a 1 second interval.  

The STREAM bandwidth tests of netperf normally rely on the kernel for ARP and routing.  
When attempting to measure UDP streaming performance over bridged interfaces to the container, however the client system was, initially, not able to establish a connection, displaying an error that indicated a routing problem.
The cause of this error is a setting in the \texttt{netperf} UDP STREAM test that apparently disables IP routing for that test by default \autocite{stackoverflownetperf}.  
This effect only shows when routing to a different subnet is required.  
For each of the network tests performed here, both bridged and physical interfaces are assigned to each virtual system to expedite testing.
In order to maintain consistency between interfaces, all UDP STREAM tests enabled routing.
...
The default message size of 64KB is fragmented into smaller data packets to fit within the default Ethernet frame size of 1500 bytes.
...
Although the network connections between the test systems did not carry any additional traffic, packet loss under the test conditions is difficult to predict.
To avoid the issue of unpredictable packet losses having a significant effect on bandwidth, the UDP STREAM tests were all run with a non-standard message size of 1472 byts with the flag \texttt{-m 1472}.  

For each of the request-response (RR) tests that were measured, the size of the messages exchanged may be set at runtime with the command line parameter \texttt{-- -r 1,1}, which sets the payload size as one byte for each direction.
