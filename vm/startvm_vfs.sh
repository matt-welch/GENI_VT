...
INTERFACE_LIST=" virtio_net ixgbevf igbvf "  # ixgbevf 
NET_OPTS=""
if [[ $INTERFACE_LIST == *"igbvf"* ]] ; then 
    # one virtual function from eth1 for internet/control (enable_vfs)
    DEVICE="09:10.1"
    if [ -z "$(lspci | grep $DEVICE)" ] ; then 
        echo "$0: ERROR: Control network interface device <$DEVICE> not available."
        exit
    fi
    NET_CONTROL="-net none -device pci-assign,host=${DEVICE} " 
...
if [[ $INTERFACE_LIST == *" ixgbe "* ]] ; then 
    ...
    DEVICE="$P258P1_PF"
    if [ -z "$(lspci | grep $DEVICE)" ] ; then 
        echo "$0: ERROR: Data network interface device <$DEVICE> not available."
        exit
    fi
    NET_DATA="-device pci-assign,host=${DEVICE} "   
    NET_OPTS="$NET_OPTS $NET_DATA"
fi
if [[ $INTERFACE_LIST == *" ixgbevf "* ]] ; then 
    # one virtual function from eth10 for VF-data (enable_vfs)
    ETH10VF0="04:10.0"
    # one virtual function from eth11 for VF-data (enable_vfs)
    ETH11VF0="04:10.1"
    NET_DATA_VF0="-device pci-assign,host=${ETH10VF0} "   
    NET_DATA_VF1="-device pci-assign,host=${ETH11VF0} "   
    NET_OPTS="$NET_OPTS $NET_DATA_VF0 $NET_DATA_VF1 "
fi
