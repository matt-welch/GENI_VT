...
Kernel CONFIG modifications are described in Table XXYY.  
...
Another important mechanism for tuning a kernel is to modify the parameters passed to the kernel at boot time \autocite{_linux_kernel_params}.  
...
For optimal performance inside our performance-tuned virtual machine we launch qemu with some additional command-line arguments to improve the guest and hypervisor performance.  
...
Given the experimental setup described in \S \ref{sec:experimental_setup}, the performance of virtual machines and containers could be compared.  
