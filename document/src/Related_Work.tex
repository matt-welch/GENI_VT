...
Traditional virtualization uses have mostly used virtual machines with containers having a secondary role as a process isolation mechanism.
Recent growth in containers, specifically the very popular Docker, has fueled a resurgence in their uses in the enterprise.
...
Growth has been strong in cloud services for good reason as outlined in \autocite{_younge_1}.
...
Since the cloud is really just a collection of virtual systems running in datacenters, HPC can potentially also reap the benefits of innovations in the cloud.
...
In their survey of network virtualization \autocite{_chowdhury_1}, Chowdhury and Boutaba describe some of the older technilogies and challenges that define this problem space.
...
In order to meet their processing target window of only 3 µs, any latency due to network processing must be minimized.
...
The third popular virtualization method is what appears to be the \emph{de facto} standard as virtualization matures.
...
The Linux subsystems supporting containers have become sophisitcated enough that Docker or LXC are no longer strictly necessary to create containers, because administrators can create their containers with native Linux  tools such as SystemD or just scripts that stitch together the correct cgroups, chroot, and namespace allocation.  
...
Although, Docker may not be representative of container systems as a whole, the majority of the subsystems used by Docker to instantiate containers is part of the Linux kernel so it is expected that containers utilizing these technologies should perform similarly.  
...
The performance degradation of of Xen virtual machines was blamed on intensive network I/O which could be helped by static allocatino and physical device assignment.
...
They show that, as streaming data uses increase in the Internet, the need for additional middleboxes to process and handle the traffic is also increasing.  These systems are increasingly assigned to virtual machines due to the flexibility and scaling that can be acheived with increased density.
...
They propose that the role of middleboxes in the network comprose the critical usability and security considerations of network design.:w
...
While this seems shockingly high, it also presents an interesting alternative use case for contaienrs and virtual machines which may be pipelined and even potentially share resources to decrease the redundancy present in current schema.
...
Containers lack the necessary isolation for co-located workloads, but, since HPC workloads and cluster topologies are often more controllable and predicatable, the lack of isolation in containers should not pose a significant problem.
...
That comparison, specifically since it was targeted at HPC, would have benefitted from more rigorous CPU isolation and tuning.  
...
Rathore [A61] compared KVM and LXC performance and isolation in hardware-assisted virtual routers, but used dynamic allocation which can kill latency \autocite{rathore2013kvm}.
...
Using VMware herein was shown to be compicated by the need to clear VMware results with them and the fact that it is proprietary and not open source all discourage its use for academic research.  
The populare Xen hypervisor, while open source, requires paravirtualization which creates additional burden.
VirtualBox, while widely available, is also not open source and has little available perfornace data for the Type 2 hypervisor.  
...
This observatin highlights the need for performant network interfaces due to the high overhead required in maintaining TCP flows.


% section comparisons_performance_analysis (end)

\nocite{_dpdk_1, _adams_1, _chowdhury_1, _perry_1, _grinberg_1, rathore2013kvm, seo2014performance, gomes2014performance}
